...
| `geni matches list [-collection X] [-filter Y] [-order Z] [-direction D] [-page N \| -all] [-limit N] [-new]` | List the merge-center matches (the OAuth API has no equivalent). Output is a JSON array of `{profile_guid, name, profile_url, lifespan_text, deceased, privacy, relationship, manager_name, manager_profile_url, updated_at_text, tree_match_count, record_match_count, smart_match_count, smart_match_value}`. `-collection` is one of `managed,relatives,followed,collaborators`; `-filter` one of `tree,record,smart`; `-order` one of `name,relationship,manager,updated_at,matches`; `-direction` `asc\|desc`. `-all` paginates until exhausted; `-limit` caps total rows; `-new` keeps only rows whose tree+record+smart counts are non-zero. |
